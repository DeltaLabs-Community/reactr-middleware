...
class MiddlewareExecutor {
  ...
  static createReactRouterLoader(middlewares: Middleware[], parallel: boolean = false, rejectOnError: boolean = false, redirect?: string) {
  ...
  export function createLoader(middlewares: Middleware[] = [], options?: { parallel?: boolean, rejectOnError?: boolean, redirect?: string }) {
    return MiddlewareExecutor.createReactRouterLoader(middlewares, options?.parallel, options?.rejectOnError, options?.redirect);
  }
  ...
  class MiddlewareRegistry {
    ...
    static get(name: string): Middleware[] {
      ...
      if (!middlewares) {
        throw new Error(`Middleware group "${name}" not found. Available groups: ${Array.from(this.middlewareGroups.keys()).join(', ')}`);
      }
      return middlewares;
    }
    ...
    static createLoader(name: string | string[], parallel: boolean = false, rejectOnError: boolean = false, redirect?: string) {
      if (Array.isArray(name)) {
        return MiddlewareExecutor.createReactRouterLoader(this.createLoaderFromGroup(name), parallel, rejectOnError, redirect);
      }
